...
var Mine = function  (ele,faceele,panewidth,paneheight,minenum,tagele,timeele) {
 ...
 Mine.prototype = {
    ...
    this.faceele.src = "img/face_normal.gif";
	...
	drawCell:function(pos,type){//绘制不同种类的格子。
		...
		var image = new Image();
		var src;
		var srcArr = ["img/blank.gif","img/0.gif","img/flag.gif","img/ask.gif","img/mine.gif","img/blood.gif","img/error.gif"];
	...
	numToImage:function (num,ele){
		...
		for (var i = 0,eLen=ele.length; i < eLen; i++) {
			ele[i].src="img/d"+num.toString().charAt(i)+".gif";
		};
